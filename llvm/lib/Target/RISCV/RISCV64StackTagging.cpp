//===- RISCV64StackTagging.cpp - Stack tagging in IR --===//
//
...
#define DEBUG_TYPE "riscv-stack-tagging"

static constexpr unsigned kTagGranuleSize = 16;

namespace llvm {
  void initializeRISCV64StackTaggingPass(PassRegistry &Registry);
} // end namespace llvm;

namespace {
class RISCV64StackTagging : public FunctionPass {
  struct AllocaInfo {
    AllocaInst *AI;
...
public:
  ...
  RISCV64StackTagging() : FunctionPass(ID) {
    initializeRISCV64StackTaggingPass(*PassRegistry::getPassRegistry());
  }
  ...
  StringRef getPassName() const override { return "RISCV-I64 Stack Tagging"; }
...
char RISCV64StackTagging::ID = 0;

INITIALIZE_PASS_BEGIN(RISCV64StackTagging, DEBUG_TYPE, "RISCV-I64 Stack Tagging",
                      false, false)
INITIALIZE_PASS_END(RISCV64StackTagging, DEBUG_TYPE, "RISCV-I64 Stack Tagging",
                    false, false)

FunctionPass *llvm::createRISCV64StackTaggingPass() {
  return new RISCV64StackTagging();
}

bool RISCV64StackTagging::isInterestingAlloca(const AllocaInst &AI) {
...
void RISCV64StackTagging::alignAndPadAlloca(AllocaInfo &Info) {
...
bool RISCV64StackTagging::runOnFunction(Function &Fn) {
  ...
  SetTagFunc =
      Intrinsic::getDeclaration(F->getParent(), Intrinsic::riscv_settag);
